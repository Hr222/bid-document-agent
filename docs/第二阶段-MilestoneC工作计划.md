...
- 当前向量检索仍以 `pgvector + cosine distance` 的精确检索作为 `exact` 基线路径
- 代码中已补齐 `exact / hnsw` 检索策略切换结构，后续主要是继续验证效果与可维护性
- 数据库 schema 与增量 SQL 中已补齐 HNSW 索引落地方案
- 配置中已补齐 HNSW 相关参数与检索策略开关
- 当前测试已覆盖策略切换与 HNSW 查询路径，但还不足以单独完成 C 阶段的 baseline / benchmark 验收
- 阶段文档与代码状态需要继续保持同步
...
当前状态：已完成，用于统一当前阶段目标与 Milestone C 主线认知。
...
当前状态：已完成，已形成 `exact / hnsw` 的策略入口与调用链路。
...
### Step C3：HNSW 索引接入与配置补齐 [已完成]

当前状态：已完成，已补齐 HNSW 索引、相关配置项与查询实现。
...
### Step C4：baseline / benchmark 与验收 [待开始]

当前状态：待开始，下一步进入效果对比与性能验收。
