...
class ChecklistEvaluationResult:
    ...
    def used_field_labels(self) -> list[str]:
        return [item.evidence.definition.label for item in self.decisions if item.submitted]

    @property
    def missing_field_labels(self) -> list[str]:
        return [item.evidence.definition.label for item in self.decisions if not item.submitted]
        ...
        for evidence in rule_pack.requirements:
            ...
            decisions.append(
                ChecklistRequirementDecision(
                    evidence=evidence,
                    submitted=not missing_components,
